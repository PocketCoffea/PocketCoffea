...
def get_systematic_uncertainty(stack_mc, variations):
    if stack_mc[0].ndim != 2:
        sys.exit("Only histograms with a dense axis and the variation axis are allowed as input.")
    ...
    for h in stack_mc:
        ...
        syst_err2_up   += np.sqrt(syst_err2_up)
        syst_err2_down += np.sqrt(syst_err2_down)
            
    return np.sqrt(syst_err2_up), np.sqrt(syst_err2_down)

def plot_systematic_uncertainty(stack_mc_nominal, syst_err_up, syst_err_down, ax):
    h_mc_sum = stack_sum(stack_mc_nominal)
    nom = h_mc_sum.values()
    up   = nom + syst_err_up
    down = nom - syst_err_down
    #unc_band = np.array([down, up])
    unc_band = np.nan_to_num( np.array([down, up]), nan=0 )

    #print("up", up)
    #print("nom", nom)
    #print("down", down)

    ax.fill_between(h_mc_sum.axes[0].edges, np.r_[unc_band[0], unc_band[0, -1]], np.r_[unc_band[1], unc_band[1, -1]], **opts_unc)

def plot_data_mc_hist1D(h, histname, config):
    '''This function plots a 1D histogram.'''
    ...
    for year in years:
        ...
        for cat in categories:
            ...
            syst_err_up, syst_err_down = get_systematic_uncertainty(stack_mc, variations)
            plot_systematic_uncertainty(stack_mc_nominal, syst_err_up, syst_err_down, ax)
            ratio, unc = get_ratio(stack_data, stack_mc_nominal)
            rax.errorbar(x, ratio, unc, linewidth=0, marker='.', markersize=5, elinewidth=1, color='black')
            ax.set_yscale('log')
            ax.set_ylim( (0,1.05*max(stack_sum(stack_mc_nominal).values())) )
            ax.legend()
...
def plot_shapes_comparison(df, var, shapes, title=None, ylog=False, output_folder=None,
                           figsize=(8,9), dpi=100, lumi_label="$137/fb$ (13 TeV)",
                           outputfile=None):
    '''
