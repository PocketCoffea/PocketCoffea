...
def plot_variation_correctionlib(file, axis_x, systematics, plot_dir, **kwargs):
    ...
    for syst in systematics:
        ...
        hep.cms.lumitext(text=f'{totalLumi}' + r' fb$^{-1}$, 13 TeV,' + f' {kwargs["year"]}', fontsize=18, ax=ax)
        ...
        ax.errorbar(x, nominal, yerr=yerr, xerr=xerr, label="SF nominal", **opts_sf['nominal'])
        xlim = (edges_x[0], edges_x[-1])
        xticks = None
        if kwargs['histname'] in config.plot_options['scalefactor'][kwargs['year']][kwargs['cat']].keys():
            ylim   = config.plot_options['scalefactor'][kwargs['year']][kwargs['cat']][kwargs['histname']]['ylim']
        else:
            ylim = (0.7, 1.3)
        if kwargs['histname'] in config.plot_options['ratio'][kwargs['year']][kwargs['cat']].keys():
            ylim_ratio = config.plot_options['ratio'][kwargs['year']][kwargs['cat']][kwargs['histname']]['ylim']
        else:
        ...
        if (syst != 'nominal'):
            ...
            linesDown  = ax.errorbar(x, systDown, yerr=0, xerr=xerr, label=f"SF {syst}Down", **opts_sf['Down'])
            linesUp    = ax.errorbar(x, systUp, yerr=0, xerr=xerr, label=f"SF {syst}Up", **opts_sf['Up'])
            rlinesDown = rax.errorbar(x, ratioDown, yerr=0, xerr=xerr, **opts_sf['Down'])
            rlinesUp   = rax.errorbar(x, ratioUp, yerr=0, xerr=xerr, **opts_sf['Up'])
            for lines, var in zip([linesDown, linesUp, rlinesDown, rlinesUp], ['Down', 'Up', 'Down', 'Up']):
                errorbar_x = lines[-1][0]
                errorbar_y = lines[-1][1]
                errorbar_x.set_linestyle(opts_errorbar[var]['linestyle'])
                errorbar_y.set_linewidth(0)
            rax.fill_between(edges_x, np.r_[unc_band[0], unc_band[0, -1]], np.r_[unc_band[1], unc_band[1, -1]], **opts_unc)
        ...
        ax.set_xlim(*xlim)
        ax.set_ylim(*ylim)
        filename = os.path.join(plot_dir, f"{kwargs['histname']}_{kwargs['year']}_sf_{kwargs['cat']}_{syst}.png")
