...
Once the output `.coffea` file has been produced, plots can be generated by executing the plotting script:
...
- `-op`: Config `.yaml` file to overwrite the plotting style (see below).
