...
class PartonMatchingProcessor(ttHbbBaseProcessor):
    def __init__(self,cfg) -> None:
        ...
        self.add_column_accumulator("npartons_matched", categories=["4j"], store_size=False)
        self.add_column_accumulator("matched_parton_pt", categories=['4j'], store_size=False)
        self.add_column_accumulator("matched_parton_eta", categories=['4j'], store_size=False)
        self.add_column_accumulator("matched_parton_phi", categories=['4j'], store_size=False)
        self.add_column_accumulator("matched_parton_pdgId", categories=['4j'], store_size=False)
        self.add_column_accumulator("matched_parton_prov", categories=['4j'], store_size=False)
        ...
        self.add_column_accumulator("matched_jet_pt", categories=['4j'], store_size=True)
        self.add_column_accumulator("matched_jet_eta", categories=['4j'], store_size=False)
        self.add_column_accumulator("matched_jet_phi", categories=['4j'], store_size=False)
        self.add_column_accumulator("matched_jet_btag", categories=['4j'], store_size=False)
        self.add_column_accumulator("matched_parton_jet_dR", categories=['4j'],store_size=False)
    ...
    def do_parton_matching(self) -> ak.Array:
        ...
        if self._sample == "ttHTobb":
            prov = get_partons_provenance_ttHbb(ak.Array(quarks.pdgId, behavior={}), ak.ArrayBuilder()).snapshot()
        else:
    ...
    def count_partons(self):
        ...
        self.events["nPartonMatched"] = ak.count(self.events.PartonMatched.pt, axis=1) # use count since we have None
    ...
    def process_extra_after_presel(self) -> ak.Array:
        ...
        # All the partons
        fill_column_accumulator(self.output["columns"], self._sample, "npartons_tot", ["4j"],
                                self.events.nParton, save_size=False, flatten=False)
        
        fill_column_accumulator(self.output["columns"], self._sample, "parton_pt", ["4j"],
                                self.events.Parton.pt,
                                save_size = False) 
        
        fill_column_accumulator(self.output["columns"], self._sample, "parton_eta", ["4j"],
                                self.events.Parton.eta,
                                save_size = False)
        
        fill_column_accumulator(self.output["columns"], self._sample, "parton_phi", ["4j"],
        ...
        fill_column_accumulator(self.output["columns"], self._sample, "parton_prov", ["4j"],
                                self.events.Parton.provenance,
                                save_size = False)

        #Matched partons
        fill_column_accumulator(self.output["columns"], self._sample, "npartons_matched", ["4j"],
                                self.events.nPartonMatched, save_size=False, flatten=False)
        
        fill_column_accumulator(self.output["columns"], self._sample, "matched_parton_pt", ["4j"],
                                ak.fill_none(self.events.PartonMatched.pt,-999),
                                save_size = False) 
        
        fill_column_accumulator(self.output["columns"], self._sample, "matched_parton_eta", ["4j"],
                                ak.fill_none(self.events.PartonMatched.eta, -999),
                                save_size = False)
        
        fill_column_accumulator(self.output["columns"], self._sample, "matched_parton_phi", ["4j"],
                                ak.fill_none(self.events.PartonMatched.phi, -999),
                                save_size = False)
        
        fill_column_accumulator(self.output["columns"], self._sample, "matched_parton_pdgId", ["4j"],
                                ak.fill_none(self.events.PartonMatched.pdgId, -999),
                                save_size = False)

        fill_column_accumulator(self.output["columns"], self._sample, "matched_parton_prov", ["4j"],
        ...
        fill_column_accumulator(self.output["columns"], self._sample, "jet_pt", ["4j"],
                                ...
                                save_size = True) # Saving also the size for all the others

        fill_column_accumulator(self.output["columns"], self._sample, "jet_eta", ["4j"],
                                ak.fill_none(self.events.JetGood.eta,-999),
                                save_size = False)
        
        fill_column_accumulator(self.output["columns"], self._sample, "jet_phi", ["4j"],
        ...
        # Matched jets
        fill_column_accumulator(self.output["columns"], self._sample, "matched_jet_pt", ["4j"],
                                ak.fill_none(self.events.JetGoodMatched.pt, -999),
                                save_size = True) # Saving also the size for all the others

        fill_column_accumulator(self.output["columns"], self._sample, "matched_jet_eta", ["4j"],
                                ak.fill_none(self.events.JetGoodMatched.eta,-999),
                                save_size = False)
        
        fill_column_accumulator(self.output["columns"], self._sample, "matched_jet_phi", ["4j"],
                                ak.fill_none(self.events.JetGoodMatched.phi,-999),
                                save_size = False)

        
        fill_column_accumulator(self.output["columns"], self._sample, "matched_jet_btag", ["4j"],
                                ak.fill_none(self.events.JetGoodMatched.btagDeepFlavB,-999),
                                save_size = False)
        
        fill_column_accumulator(self.output["columns"], self._sample, "matched_parton_jet_dR", ["4j"],
