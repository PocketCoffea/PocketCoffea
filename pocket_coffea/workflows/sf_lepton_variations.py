...
class sfLeptonVariationsProcessor(ttHbbBaseProcessor):
    def __init__(self,cfg) -> None:
        ...
        for histname, h in self._hist_dict.items():
            varname = h.fields[-1]
            self._hist_dict[histname] = hist.Hist("$N_{events}$", *h.axes(), sf_ele_reco_axis,
                                                  sf_ele_id_axis, sf_mu_id_axis, sf_mu_iso_axis)
    ...
    def fill_histograms(self):
        ...
        for (obj, obj_hists) in zip([None], [self.nobj_hists]):
            fill_histograms_object_with_variations(self, obj, obj_hists, variations, event_var=True)
