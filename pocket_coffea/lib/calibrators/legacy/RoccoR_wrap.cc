...
#include <stdexcept>
#include <cstring>
#include "RoccoR_standalone.h"

const double CrystalBall::pi = 3.14159265358979323846;
const double CrystalBall::sqrtPiOver2 = sqrt(CrystalBall::pi/2.0);
const double CrystalBall::sqrt2 = sqrt(2.0);

// ---- paste original RoccoR.cc implementation (without the boost include / TString) ----

RocRes::RocRes(){ reset(); }

void RocRes::reset(){
    NETA=0; NTRK=0; NMIN=0;
...
void RoccoR::init(std::string filename){
    ...
    while(std::getline(in, s)){
        ...
        if(s.substr(0,7)=="VERSION") continue;
        std::string first4=s.substr(0,4);
        if(first4=="NSET"){ ss >> tag >> nset; nmem.resize(nset); tvar.resize(nset); RC.resize(nset); }
        else if(first4=="NMEM"){ ss >> tag; for(int i=0; i<nset; ++i){ ss >> nmem[i]; RC[i].resize(nmem[i]); } }
        else if(first4=="TVAR"){ ss >> tag; for(int i=0; i<nset; ++i) ss >> tvar[i]; }
        else if(first4=="RMIN") ss >> tag >> RMIN;
        else if(first4=="RTRK") ss >> tag >> RTRK;
        else if(first4=="RETA"){ ss >> tag >> RETA; BETA.resize(RETA+1); for(auto &h: BETA) ss >> h; }
        else if(first4=="CPHI"){ ss >> tag >> NPHI; DPHI=2*CrystalBall::pi/NPHI; }
        else if(first4=="CETA"){ ss >> tag >> NETA; etabin.resize(NETA+1); for(auto& h: etabin) ss >> h; }
        else{ 
            ss >> sys >> mem >> tag;
            auto &rc = RC[sys][mem]; 
            rc.RR.NETA=RETA; rc.RR.NTRK=RTRK; rc.RR.NMIN=RMIN;
            ...
            auto &cp = rc.CP;
            for(TYPE T:{MC,DT}){
                if(cp[T].empty()){ cp[T].resize(NETA); for(auto &i: cp[T]) i.resize(NPHI); }
            }
            if(tag=="R"){
                ss >> var >> bin; 
            ...
            else if(tag=="T"){ ss >> type >> bin; for(int i=0; i<RTRK+1; ++i) ss >> resol[bin].nTrk[type][i]; }
            else if(tag=="F"){ ss >> type; for(int i=0; i<RETA; ++i) ss >> resol[i].kRes[type]; }
            else if(tag=="C"){
                ss >> type >> var >> bin; 
...
double RoccoR::kScaleDT(int Q, double pt, double eta, double phi, int s, int m) const{
    int H = etaBin(eta); int F = phiBin(phi);
...
// =====================================================================
// C-style extern "C" wrapper so ctypes can call these directly
// =====================================================================
extern "C" {

    RoccoR* RoccoR_new(const char* filename){
        return new RoccoR(std::string(filename));
    }

    void RoccoR_delete(RoccoR* rc){
        delete rc;
    }

    double RoccoR_kScaleDT(RoccoR* rc, int Q, double pt, double eta, double phi, int s, int m){
        return rc->kScaleDT(Q, pt, eta, phi, s, m);
    }

    double RoccoR_kSpreadMC(RoccoR* rc, int Q, double pt, double eta, double phi, double gt, int s, int m){
        return rc->kSpreadMC(Q, pt, eta, phi, gt, s, m);
    }

    double RoccoR_kSmearMC(RoccoR* rc, int Q, double pt, double eta, double phi, int n, double u, int s, int m){
        return rc->kSmearMC(Q, pt, eta, phi, n, u, s, m);
    }

    double RoccoR_kScaleDTerror(RoccoR* rc, int Q, double pt, double eta, double phi){
        return rc->kScaleDTerror(Q, pt, eta, phi);
    }

    double RoccoR_kSpreadMCerror(RoccoR* rc, int Q, double pt, double eta, double phi, double gt){
        return rc->kSpreadMCerror(Q, pt, eta, phi, gt);
    }

    double RoccoR_kSmearMCerror(RoccoR* rc, int Q, double pt, double eta, double phi, int n, double u){
        return rc->kSmearMCerror(Q, pt, eta, phi, n, u);
    }

    // Vectorised versions — operate on flat C arrays, return array of scale factors
    // n_muons: number of muons
    // out: pre-allocated double array of length n_muons
    void RoccoR_kScaleDT_vec(RoccoR* rc,
                              const int* Q, const double* pt, const double* eta, const double* phi,
                              int s, int m, int n_muons, double* out){
        for(int i=0; i<n_muons; ++i)
            out[i] = rc->kScaleDT(Q[i], pt[i], eta[i], phi[i], s, m);
    }

    void RoccoR_kSpreadMC_vec(RoccoR* rc,
                               const int* Q, const double* pt, const double* eta, const double* phi,
                               const double* gt,
                               int s, int m, int n_muons, double* out){
        for(int i=0; i<n_muons; ++i)
            out[i] = rc->kSpreadMC(Q[i], pt[i], eta[i], phi[i], gt[i], s, m);
    }

    void RoccoR_kSmearMC_vec(RoccoR* rc,
                              const int* Q, const double* pt, const double* eta, const double* phi,
                              const int* n, const double* u,
                              int s, int m, int n_muons, double* out){
        for(int i=0; i<n_muons; ++i)
            out[i] = rc->kSmearMC(Q[i], pt[i], eta[i], phi[i], n[i], u[i], s, m);
    }
}
