...
// Standalone erf_inv to replace boost::math::erf_inv
namespace standalone_math {
    // Rational approximation for erf_inv, accurate to ~1e-9
    inline double erf_inv(double x) {
        // Winitzki approximation
        const double a = 0.147;
        const double pi = 3.14159265358979323846;
        double ln1mx2 = std::log(1.0 - x*x);
        double t1 = 2.0/(pi*a) + ln1mx2/2.0;
        double t2 = ln1mx2/a;
        return std::copysign(std::sqrt(std::sqrt(t1*t1 - t2) - t1), x);
    }
}
...
struct CrystalBall{
    ...
    double invcdf(double u) const{
        ...
        return m - sqrt2 * s * standalone_math::erf_inv((D - u/Ns )/sqrtPiOver2);
    }
};
