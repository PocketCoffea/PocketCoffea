...
def plot_systematic_uncertainty(stack_mc_nominal, syst_err_up, syst_err_down, ax, ratio=False):
    ...
    ax.fill_between(h_mc_sum.axes[0].edges, np.r_[unc_band[0], unc_band[0, -1]], np.r_[unc_band[1], unc_band[1, -1]], **opts_unc, label="syst. unc.")
...
def plot_data_mc_hist1D(h, histname, config):
    ...
    for year in years:
        ...
        for cat in categories:
            ...
            if not is_mc_only:
                ax.errorbar(x, h_data.values(), yerr=np.sqrt(h_data.values()), **opts_data)
            ...
            plot_systematic_uncertainty(stack_mc_nominal, syst_err_up, syst_err_down, rax, ratio=True)
