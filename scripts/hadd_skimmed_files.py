...
# Inputs
parser.add_argument('-fl','--files-list', required=True, type=str,
                    help='Parquet file containing the skimmed files metadata')
parser.add_argument("-o", "--outputdir", required=False, type=str,
                    help="Output folder")
parser.add_argument("--only-datasets", nargs="+", type=str, help="Restring list of datasets")
...
parser.add_argument("-s","--scaleout", type=int, help="Number of threads to process the hadd.", default=2 )
parser.add_argument("--overwrite", action="store_true", help="Overwrite files")
parser.add_argument("--dry", action="store_true", help="Do not execute hadd, save metadata")
...
for dataset in df["skimmed_files"].keys():
    ...
    group = []
    ngroup = 1
    for file, nevents in zip(df["skimmed_files"][dataset], df["nskimmed_files"][dataset]):
        if (args.files and (nfiles+1)>args.files)  or (args.events and (nevents_tot+nevents)>args.events):
...
dataset_definition = { s: {"files": list(d['files'].keys())}
